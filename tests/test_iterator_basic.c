...
static void test_empty_tree_iteration(void)
{
    ...
    const PTMapNodeT *n = pmapiter_next(&it);
    TEST_ASSERT_NULL(n);
...
static void test_single_node_iteration(void)
{
    ...
    const char *k = "solo";
    bool ins;
    PTMapNodeT *n = (PTMapNodeT*)patrimap_insert(&m, k, (uint16_t)(strlen(k) * 8), &ins);
    ...
    PTMapIterT it;
    pmapiter_init(&it, &m, NULL, true, ePTMode_preOrder);
    const PTMapNodeT *p = pmapiter_next(&it);
    ...
    /* reset and reverse */
    pmapiter_init(&it, &m, NULL, false, ePTMode_postOrder);
    const PTMapNodeT *r = pmapiter_next(&it);
